...
function result(){
	...
	$('.ctt').each(function(){
		...
		var userPay = Number(uncomma($(this).find('.pay').val())) / userCnt; // 항목의 총액에서 참여한 인원만큼 나눈 값
		$(this).find('input[type="checkbox"]:checked').val(userPay); // 항목의 총액에서 참여한 인원만큼 나눈 값을 각 체크박스 val 에 할당
		result += $(this).find('.place').val() + '<br>' + $(this).find('.pay').val() + '원 / ' + userCnt  + ' = ' + userPay.toLocaleString('en') + '원<br><br>'; // 지출장소 <br> 총액 / 참여한 인원 = 인원당 청구 금액
	});	

	// 각 유저에게 할당
	$('input:checked').each(function(){
		var ele = $(this).attr('class') // 해당 체크박스의 Class 를 담고
		if ($('.'+ele+':checked').eq(0).attr('id') == $(this).attr('id')){ // 해당 체크박스의 ID와 '체크된 체크박스 중 첫번째 체크박스의 ID' 같다면
			if (userTotal($(this).attr('class')) !== 0){
				result += $(this).next().html() + ' = ' + (Math.round(userTotal($(this).attr('class'))/1000)*1000).toLocaleString('en') + '원 <span style="font-size: 0.9em;color: #999;">(백원단위 반올림)</span><br>';
			}
		}
	});
