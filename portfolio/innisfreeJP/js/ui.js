...
$(document).on('click','#gnb_util .u2 > a',function(){
	if ($(this).hasClass('active')) {
		$(this).removeClass('active');
		$(this).next('.sub_my').stop().slideUp(200);
	} else {
		$(this).addClass('active');
		$(this).next('.sub_my').stop().slideDown(200);
	}
	return false;
});
$(document).on('mouseleave','#gnb_util .sub_my',function(){
...
$(document).on('click', '#btn_goSch', function(){
	$('#btn_search').click()
	$('#gSearch').focus();
});
...
$(document).on('click', '#schArea, #schArea .btn_close', function(){
	$('html').css('overflow','');
	$('#btn_search').removeClass('active');
	$('#schArea').removeClass('visible');
	return false;
});
$(document).on('click', '#schArea .schAreaIn', function(e){
	e.stopPropagation();
});
...
$(document).on('click', '#cartArea .btn_close, .mask', function(){
	$('html').css('overflow','');
...
$(document).on('click', '.inp_select .title', function(){
	if ($(this).parent('.inp_select').hasClass('disabled')) {return false;}
	...
	if (target.hasClass('open')) {
		...
		$(this).parent('.inp_select').addClass('open');
...
$(document).on('click', '.inp_select .selList label', function (){
	...
	*/
	...
	var target = $(this).parents('.inp_select');
	...
	target.find('.inp_val').val($(this).prev('input').val());
});
/*
$(document).on('mouseleave', '.inp_select', function(){
	var _target = $(this);
...
sortType
**/
$(document).on('click', '.sortType .title', function(){
	if ($(this).parent('.sortType').hasClass('disabled')) {return false;}

	var target = $(this).parent();

	if (target.hasClass('open')) {
		$(this).siblings('.sortList').stop().animate({'maxHeight' : 0+'px'}, 250, 'easeOutCubic');
		setTimeout(function(){
			target.find('.sortList').hide();
			target.removeClass('open');
		},250);			
	} else {
		//$('.sortType.open .title').click();
		$(this).parent('.sortType').addClass('open');
		$(this).siblings('.sortList').show();
		$(this).siblings('.sortList').stop().animate({'maxHeight' : 250+'px'}, 500, 'easeOutCubic');
	};
});

$(document).on('click', '.sortType .sortList label', function (){
	/*
	var disabled = $(this).prev('input').prop('disabled');
	if (disabled) {return false}
	*/
	...
	var target = $(this).parents('.sortType');
	target.find('.title').text(txt_value);
	target.find('input').removeAttr('checked');
	$(this).prev('input').attr('checked','checked');
	$(this).parents('.sortList').stop(true, false).animate({'maxHeight' : 0+'px'}, 250, 'easeOutCubic');
	setTimeout(function(){
		target.removeClass('open');
		target.find('.sortList').hide();
	},250);
...
BTN
**/
$('.btn_inherit').hover(function(){
	console.log('tt');
	var color = $(this).find('.bar').css('background-color');
...
function delPdtCompare(e){
	$(e).parent().remove();
	$('#lyPdtCompare .pdtCompareList').append('<div class="item"></div>');
	$('#lyPdtCompare .n').text($('#lyPdtCompare .n').text()-1);
}
function resetPdtCompare(){
	$('#lyPdtCompare .pdtCompareList').html('<div class="item"></div><div class="item"></div><div class="item"></div>');
...
$(document).on('click', '.rating_star_large .starPoint button', function(){
	...
	$('.rating_star_large .innerPoint').html(point + '.0');
});
...
$(".top-open").click(function(){
	if(!navOn){
		$('#header').addClass("on");
		$('.title_wrap').addClass("gnb_on");
		$("#teaserGnb").show().stop().animate({opacity:1},300);
		...
		$('.title_wrap').removeClass("gnb_on");
		$("#teaserGnb").stop().animate({opacity:0},300,function(){
			$(this).hide()
		});
...
function ttlFixed (e){	
	if ($(e).scrollTop() > $('.title_wrap').offset().top + 40){
		$('.title_wrap').addClass('nav-down');
	} else {
		$('.title_wrap').removeClass('nav-down');
	}
}
